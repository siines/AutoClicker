...
// Lerp between colors (Old - can be removed or kept if used elsewhere, removing for now)
            ...
            if (selectedInputType == KEYBOARD_INPUT) {
                ...
                if (waitingForKeyboardKey) {
                    ...
                    if (elapsedTime < transitionTime) {
                        ...
                        ImVec4 transitionColor = LerpColor(ImVec4(0.3f, 0.2f, 0.2f, 1.0f), ImVec4(0.2f, 0.3f, 0.2f, 1.0f), g_keyboardKeyReadyProgress);
        ...
        if (ImGui::BeginTabItem("Hotkeys")) {
            ...
            if (g_isSettingHotkey.load()) {
                ...
                if (elapsedTime < transitionTime) {
                    ...
                    ImVec4 transitionColor = LerpColor(ImVec4(0.3f, 0.2f, 0.2f, 1.0f), ImVec4(0.2f, 0.3f, 0.2f, 1.0f), g_hotkeyReadyProgress);
                    ImGui::PushStyleColor(ImGuiCol_Button, transitionColor);
                    ImGui::Button("Initializing...", ImVec2(150, 0));
                    ImGui::PopStyleColor();
                    ImGui::SameLine();
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_SYSCOMMAND: {
            ...
            // Handle context menu commands from tray icon
            if (wParam == 1) { // Restore (Menu item ID 1)
                ShowWindow(hWnd, SW_RESTORE);
                SetForegroundWindow(hWnd);
                RemoveTrayIcon(hWnd);
                g_isMinimizedToTray = false;
                return 0;
            } else if (wParam == 2) { // Exit (Menu item ID 2)
                g_immediateQuit = true; // Signal threads to stop
                RemoveTrayIcon(hWnd);   // Clean up tray icon immediately
