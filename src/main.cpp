...
void RenderUIWindow() {
    ...
    if (ImGui::InputInt("Interval (ms)", &currentInterval, 10, 50)) {
        ...
        SaveSettings(); // Save settings when interval changes
    }
    ImGui::SameLine(); 
    ...
    if (ImGui::Checkbox("Lock Cursor Position", &lockCursor)) {
        ...
        SaveSettings(); // Save settings when lock cursor option changes
    }
    ImGui::SameLine(); 
    ...
    if (g_isSettingHotkey.load()) {
        // Calculate the transition progress based on time
        DWORD elapsedTime = GetTickCount() - g_hotkeyButtonPressTime;
        const DWORD transitionTime = 200; // ms for full transition
        ...
        if (elapsedTime < transitionTime) {
            ...
            // Use a more subtle color transition
            ImVec4 transitionColor = LerpColor(
                ImVec4(0.3f, 0.2f, 0.2f, 1.0f),  // Start color (subtle red)
                ImVec4(0.2f, 0.3f, 0.2f, 1.0f),  // End color (subtle green)
                g_hotkeyReadyProgress
            );
            ...
            // Show a subtle progress indicator instead of text
            ImGui::SameLine();
            ...
            // Ready to listen - more subtle pulsing
            g_hotkeyReadyProgress = 1.0f;
            g_hotkeyReadyToListen = true;
            
            // Use a subtle pulsing effect
        ...
        if (ImGui::Button(hotkey.c_str(), ImVec2(100, 0))) {
            // Reset progress and timestamps when button is clicked
    ...
    if (ImGui::BeginCombo("##InputType", inputTypeNames[selectedInputType])) {
        for (int i = 0; i < 2; i++) {
            ...
            if (ImGui::Selectable(inputTypeNames[i], isSelected)) {
                ...
                SaveSettings(); // Save settings when input type changes
            }
        ...
        if (ImGui::BeginCombo("##MouseButton", mouseButtonNames[selectedMouseButton])) {
            for (int i = 0; i < 3; i++) {
                ...
                if (ImGui::Selectable(mouseButtonNames[i], isSelected)) {
                    ...
                    SaveSettings(); // Save settings when mouse button changes
                }
    ...
    if (selectedInputType == KEYBOARD_INPUT) {
        ...
        if (waitingForKeyboardKey) {
            ...
            if (elapsedTime < transitionTime) {
                // Still in transition period - more subtle
